...
//  PDFEditor
//
//  Created by Itsuki on 2025/01/03.
//
...
struct DrawingView: View {
    ...
    var body: some View {
        PDFViewRepresentable()
            ...
            .overlay(content: {
                Rectangle()
                    ...
                    .gesture(
                        DragGesture(minimumDistance: 0.0, coordinateSpace: .local)
                            .onChanged { value in
                                ...
                                let bounds = CGRect(origin: .zero, size: CGSize(width: pdfManager.pageBounds.width, height: pdfManager.pageBounds.height))
                    ...
                    .frame(width: pdfManager.pageBounds.width, height: pdfManager.pageBounds.height)
                    .position(CGPoint(x: pdfManager.pageBounds.origin.x + pdfManager.pageBounds.width/2 , y: pdfManager.pageBounds.origin.y + pdfManager.pageBounds.height/2))
            })
        .ignoresSafeArea(.all)
            ...
            HStack {
                if viewMode == .view {
                    ...
                    Button(action: {
                        ...
                        eraserLocation = CGPoint(x: pdfManager.pageBounds.width / 2, y: pdfManager.pageBounds.height / 2)
                    }, label: {
                        Text("Erase")
                    })
                    .foregroundStyle(.black)
        ...
        .onAppear {
            if let url = Bundle.main.url(forResource: "differentPageSize", withExtension: "pdf"), let data = try? Data(contentsOf: url) {
